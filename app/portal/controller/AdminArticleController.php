...
class AdminArticleController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $this->success('添加成功!', url('AdminArticle/index',array('category'=>$data['post']['categories'])));
        }
    ...
    public function edit()
    {
        ...
        #lx-start
        if ($post['post_keywords']){
            $tarr = explode(',', $post['post_keywords']);
            $tags = [];
            foreach ($tarr as $tag){
                array_push($tags, Db::name('portal_tag')->where('name',$tag)->value('id'));
            }
            $tags = implode(',', $tags);
            $post['post_tags'] = $tags;
        }
    ...
    public function editPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->success('保存成功!',url('AdminArticle/index',array('category'=>$data['post']['categories'])));
    ...
    {
        $terms_id = $this->request->param('old_term_id');
        $pctMod = new PortalCategoryModel();
        $terms_tree = $pctMod->adminCategoryTree($terms_id);
        $this->assign('terms_tree',$terms_tree);
        return $this->fetch();
    }
    ...
    public function autoPustToBaidu()
    {
        $list = Db::name('portal_post')->where(['delete_time'=>0])->field('id')->select();
        $urlArr = [];
        foreach ($list as $key=>$val){
            $urlArr['url'][$key] = url('portal/Article/index',array('id'=>$list[$key]['id']),'',true,true);
        }
        $urlArr = $urlArr['url'];
        
        $api = 'http://data.zz.baidu.com/urls?site=https://www.lnmpa.top&token=MaWfgDog7iswsuFi';
        $ch = curl_init();
        $options =  array(
                CURLOPT_URL => $api,
                CURLOPT_POST => true,
    ...
    public function statisticsPost()
    {
        ...
        $commentMod = Db::name('plugin_lxcomment');
        $countArr = [];
        foreach ($ids as $key=>$val){
            $countArr[$key]['comment_count'] = $commentMod->where(['object_id'=>$val])->count();
            $countArr[$key]['id'] = $val;
            if ($countArr[$key]['comment_count'] == 0){
                unset($countArr[$key]);
            }
        }
        
        if (count($countArr)==0){
            $this->error('所选文章暂无网友评论或评论内容已彻底删除！');
        }else {
            $resl = $portalPostModel->saveAll($countArr);
            if ($resl){
                $this->success('更新评论数成功！');
            }else {
                $this->error('更新失败！');
            }
        }
